...
-- Normalize trigger: use trimmed name for match and insert, and lock to prevent races
...
BEGIN
  ...
  v_name := NULLIF(TRIM(v_contract.client_name), '');
  ...
  INSERT INTO public.clients (
    ...
    v_name,
    COALESCE(NULLIF(TRIM(NEW.signer_name), ''), v_name),
...
-- Deduplicate existing clients by normalized name (keep oldest, move refs)
DO $$
DECLARE
  r RECORD;
  keep_id uuid;
...
BEGIN
  FOR r IN
    SELECT lower(trim(company_name)) n, array_agg(id ORDER BY created_at) ids
    FROM public.clients
  ...
  LOOP
    ...
    END LOOP;
    UPDATE public.clients SET company_name = trim(company_name) WHERE id = keep_id;
  END LOOP;
END $$;
